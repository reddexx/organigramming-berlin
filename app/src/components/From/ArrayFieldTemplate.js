...
const ArrayFieldTemplate = (props) => {
  const isSimpleStringArray = props.schema?.items?.type === "string";
  const [type] = useState(
    props.schema?.items?.$ref ? props.schema.items.$ref.split("/")[2] : props.idSchema.$id
  );
              ...
              >
                {(provided, snapshot) => (
                  ...
                  >
                    {isSimpleStringArray ? (
                      <>
                        <div className="flex-grow-1 px-2 py-1 align-self-center">
                          {element.children}
                        </div>
                        {element.hasRemove && (
                          <button
                            type="button"
                            className="btn btn-danger btn-sm m-1 flex-shrink-0 align-self-center"
